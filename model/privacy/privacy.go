...
type (
	...
	QueryRule interface {
		EvalQuery(context.Context, model.Query) error
	}
)

// EvalQuery evaluates a query against a query policy.
func (policy QueryPolicy) EvalQuery(ctx context.Context, q model.Query) error {
...
type (
	...
	MutationRule interface {
		EvalMutation(context.Context, model.Mutation) error
	}
)

// EvalMutation evaluates a mutation against a mutation policy.
func (policy MutationPolicy) EvalMutation(ctx context.Context, m model.Mutation) error {
	if decision, ok := decisionFromContext(ctx); ok {
		return decision
	}
	for _, rule := range policy {
...
func (policy Policy) EvalQuery(ctx context.Context, q model.Query) error {
	return policy.Query.EvalQuery(ctx, q)
}

// EvalMutation forwards evaluation to mutation policy.
func (policy Policy) EvalMutation(ctx context.Context, m model.Mutation) error {
...
func (f ExampleQueryRuleFunc) EvalQuery(ctx context.Context, q model.Query) error {
	if q, ok := q.(*model.ExampleQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *model.ExampleQuery", q)
}

// The ExampleMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type ExampleMutationRuleFunc func(context.Context, *model.ExampleMutation) error

// EvalMutation calls f(ctx, m).
func (f ExampleMutationRuleFunc) EvalMutation(ctx context.Context, m model.Mutation) error {
	if m, ok := m.(*model.ExampleMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *model.ExampleMutation", m)
}
